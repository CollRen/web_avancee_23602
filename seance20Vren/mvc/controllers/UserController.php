...
class UserController{
    ...
    public function store($data){
        ...
        $validator->field('password', $data['password'])->min(6)->max(20)->unique('User');
        $validator->field('email', $data['email'])->required()->max(100)->email();
        $validator->field('privilege_id', $data['privilege_id'],'Privilège')->required();
        ...
        if($validator->isSuccess()){
           ...
           //$data['email'] = $data['username']; Pendant le développement, pour éviter la répétition
